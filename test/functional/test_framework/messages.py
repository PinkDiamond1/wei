...
# Objects that map to weid objects, which can be serialized/deserialized
...
class CBlockHeader():
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(weihash(r))
            self.hash = encode(weihash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers():
    ...
    def deserialize(self, f):
        # comment in weid indicates these should be deserialized as blocks
