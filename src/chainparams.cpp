...
{
    const char* pszTimestamp = "WEI Coin start in 2021";
...
public:
    CMainParams() {
        ...
        consensus.DIP0008Height = 300;
        consensus.powLimit = uint256S("00000fffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        consensus.nPowTargetTimespan = 24 * 60 * 60; // Wei: 1 day
        consensus.nPowTargetSpacing = 2.5 * 60; // Wei: 2.5 minutes
        ...
        consensus.nMinimumChainWork = uint256S("0x0"); // 1450000

        // By default assume that the signatures in ancestors of this block are valid.
        consensus.defaultAssumeValid = uint256S("0x0"); // 1450000
        ...
        genesis = CreateGenesisBlock(1639208552, 738159, 0x1e0ffff0, 1, 0 * COIN);
        ...
        // Wei script addresses start with '6'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,13);
        // Wei private keys start with '6' or 'i'
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1, 230);
        // Wei BIP32 pubkeys start with 'xpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x88, 0xB2, 0x1E};
        // Wei BIP32 prvkeys start with 'xprv' (Bitcoin defaults)
...
public:
    CTestNetParams() {
        ...
        // Testnet Wei script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,19);
        // Testnet private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,239);
        // Testnet Wei BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Testnet Wei BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Testnet Wei BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
...
public:
    CDevNetParams(bool fHelpOnly = false) {
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        //vSeeds.push_back(CDNSSeedData("weievo.org",  "devnet-seed.weievo.org"));
...
public:
    CRegTestParams() {
        ...
        // Regtest Wei addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,140);
        // Regtest Wei script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,19);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,239);
        // Regtest Wei BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Regtest Wei BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Regtest Wei BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
