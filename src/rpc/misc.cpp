...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            " - \"wei\" activates all Wei-specific categories at once;\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "wei")
            + HelpExampleRpc("debug", "wei+net")
        );
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given wei address.\n"
            ...
            "\nArguments:\n"
            "1. \"address\"                    (string, required) The wei address to validate\n"
            "\nResult:\n"
            "{\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The wei address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "wei-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\nwei-cli has the option -getinfo to collect and format these in the old format."
    );
}
