...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::WEI);
    ...
    if (!settings.contains("nCoinJoinAmount")) {
        ...
        if (!settings.contains("nAnonymizeWeiAmount"))
            settings.setValue("nCoinJoinAmount", DEFAULT_COINJOIN_AMOUNT);
        else
            settings.setValue("nCoinJoinAmount", settings.value("nAnonymizeWeiAmount").toInt());
    }
