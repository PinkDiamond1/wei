...
const char * const BITCOIN_CONF_FILENAME = "wei.conf";
const char * const BITCOIN_PID_FILENAME = "weid.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "weid -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\WeiCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\WeiCore
    // Mac: ~/Library/Application Support/WeiCore
    // Unix: ~/.weicore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WeiCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/WeiCore";
#else
    // Unix
    return pathRet / ".weicore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty wei.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Wei Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Wei Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The Wei Core developers";
    }
